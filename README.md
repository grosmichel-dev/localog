...
```
content/           마크다운 문서 + 진본 데이터 CSV (이것이 진본)
└─ 대전시/대덕구/{구청공지사항,고시공고,구청장회의,구의회회의록,예산서}/
   └─ 예산서/      *.csv (진본) + *.meta.yaml — 문서는 빌드가 자동 생성
...
**저장소에 없는 것**: 카탈로그(`llms.txt`·`catalog.csv`)는 GitHub Actions가 배포할 때마다 새로 만들어 **사이트에만** 올립니다. 저장소에 커밋하지 않습니다. 예산서처럼 CSV에서 자동 생성되는 문서(`generated: true`)도 마찬가지로 커밋하지 않습니다 — 진본은 CSV 쪽입니다.
...
예산서처럼 **표가 본질인 자료**는 마크다운 대신 CSV로 올립니다 → `docs/BUDGET-HOWTO.md`
